...
console.log(splitMe('La mia labrador non è mai stanca'));

/* ESERCIZIO 4
  Crea una funzione chiamata "deleteOne" che riceve una stringa e un booleano come parametri.
  Se il valore booleano è true la funzione deve ritornare la stringa senza il primo carattere, altrimenti la deve ritornare senza l'ultimo.
*/
function deleteOne(varString, varBoolean){
   return varBoolean ? varString.slice(1, varString.length) : varString.slice(0, -1);
}
console.log(deleteOne('La mia labrador non è mai stanca', true));
...
function onlyLetters(varString){
   return varString.replace(/[0-9]+ /g, '');
}
...
*/
function isThisAnEmail(varEmail){
   let regEx = /[A-z0-9\.\+_-]+@[A-z0-9\._-]+\.[A-z]{2,6}/;

   return regEx.test(varEmail) ? true : 'E-mail inserita non valida';
}
...
*/
function rollTheDices(varNumber){
   let obj = {
      sum: 0,
      values: []     
   }
...
function deleteProp(varObject, varString){
   delete varObject[varString];
   return varObject;
}
console.log(deleteProp(dog,'age'));
...
function newestMovie(){
   ...
   for(let i = 0; i < movies.length; i++){
      ...
      if(movies[i].Year > yearFilm){
         yearFilm = movies[i].Year;
         indexFilm = i;
      }
   }
   return movies[indexFilm];
}
...
function sumAllTheYears(){
   ...
   for(let movie of movies){
      ...
      sumYears += Number.parseInt(movie.Year);
   }
...
*/
function searchByTitle(varString){
   
   let resultSearch = [];

   for(let movie of movies){

      movie.Title.includes(varString) ? resultSearch.push(movie) : null;
   }
...
*/
function searchAndDivide(varString){
   ...
   for(let movie of movies){
      ...
      movie.Title.includes(varString) ? varObject.match.push(movie) : varObject.unmatch.push(movie);
   }
...
function removeIndex(varNumber){

   movies.splice(varNumber, 1);
...
function addListItem(){
   ...
   let textLi = document.createTextNode('List Item JS');
...
function removeListItem(){
   ...
   let btnDelete = document.querySelector('button');
